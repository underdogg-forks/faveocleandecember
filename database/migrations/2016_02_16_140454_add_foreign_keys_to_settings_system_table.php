...
class AddForeignKeysToSettingsSystemTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('core__systemsettings', function (Blueprint $table) {
            $table->foreign('time_zone', 'settings_system_ibfk_1')->references('id')->on('core__timezones')->onUpdate('NO ACTION')->onDelete('RESTRICT');
            $table->foreign('core__timeformats', 'settings_system_ibfk_2')->references('id')->on('core__timeformats')->onUpdate('NO ACTION')->onDelete('RESTRICT');
