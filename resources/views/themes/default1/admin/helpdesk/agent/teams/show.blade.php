...
<div class="box box-primary">
    <div class="box-header with-border">
        ...
        @endif
        <span class="lead border-left">{!! Lang::get('lang.status') !!} :
            <?php if($teams->status == 1) { $stat = Lang::get('lang.active'); } elseif($teams->status == 0) { $stat = Lang::get('lang.inactive'); } ?>{!!
            $stat !!} </span>


        <div class="pull-right">
            <a href="{{URL::route('teams.index')}}" class="btn btn-primary"><i class="fa fa-arrow-left" aria-hidden="true"></i>
                {{Lang::get('lang.go_back')}}</a>
        </div>
    </div>
