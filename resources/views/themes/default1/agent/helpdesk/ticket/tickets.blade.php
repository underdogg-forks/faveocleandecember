    ...
    <div class="box-body ">
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu">
                ...
                <li onclick="changeStatus({!! $ticket_status -> id !!}, '{!! $ticket_status->name !!}')"><a href="#"><i
                            class="{!! $ticket_status->icon !!}" style="color:{!! $ticket_status->icon_color !!};"> </i>{!!
                        $ticket_status->name !!}</a></li>
                @endforeach
            </ul>
        </div>
...
<script>
    $(document).ready(function () { /// Wait till page is loaded
        ...
        if (JSON.parse('<?= json_encode($inputs["assigned"]) ?>') == '1' || JSON.parse(
                '<?= json_encode($inputs["assigned"]) ?>') == 1) {}
        @endif
        ...
        $('#resetFilter').on("click", function () {
            $(
                '.filter, #assigned-to-filter, #departments-filter, #sla-filter, #priority-filter, #source-filter'
...
<script type="text/javascript">
    var $dept_list = $("#departments-filter").addSelectlist({
    ...
    var $sla_list = $("#sla-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $priority_list = $("#priority-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $labels_list = $("#labels-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $tags_list = $("#tags-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $owner_list = $("#owner-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $assignee_list = $("#assigned-to-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $status_list = $("#status-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $source_list = $("#source-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $type_list = $("#type-filter").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $number_list = $("#ticket-number").addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    var $help_topic_list = $('#help-topic-filter').addSelectlist({
        maximumSelectionLength: 5
    });
    ...
    $('#filter-form').on('submit', function (e) {
        if (clearlist > 0) {
            $(
                '#departments-filter, #sla-filter, #priority-filter, #source-filter, #owner-filter, #status-filter, #assigned-filter, #assigned-to-filter, #labels-filter, #tags-filter, #type-filter, #due-on-filter, #created, #modified, #ticket-number, #help-topic-filter'
            ).remove();
            $(this).children();
        }
    });
