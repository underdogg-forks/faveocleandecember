 ...
 */
class HelpdeskUsersController extends Controller
{
    /**
...
//            $user->save();
    ...
    public function deleteAgent($id)
    {
        ...
        if ($users->role == 'agent') {
            if ($delete_all == null) {
                ...
                //     DB::table('crm__user_assign_organization')->where('user_id', '=', $id)->delete();
                // }
                $user = User::find($id);
                $users->is_delete = 1;
                $users->active = 0;
    ...
    {
        $user = new User();
        $users = $user->leftJoin('crm__user_assign_organization', 'users.id', '=', 'user_assign_organization.user_id')
