...
class SlaPlansController extends Controller
{
    ...
    public function store(Sla_plan $sla, SlaRequest $request)
    {
        try {
            ...
            return redirect('slaplans')->with('success', Lang::get('lang.sla_plan_created_successfully'));
        } catch (Exception $e) {
            /* redirect to Index page with Fails Message */
            return redirect('slaplans')->with('fails', Lang::get('lang.sla_plan_can_not_create').'<li>'.$e->getMessage().'</li>');
        }
    }
    ...
    public function edit($id)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.manage.slaplans.edit', compact('slas', 'sla'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->input('sys_sla') == 'on') {
                \DB::table('tickets__settings')
                     ->where('id', '=', 1)
                     ->update(['slaplan_id' => $id]);
            }

            return redirect('slaplans')->with('success', Lang::get('lang.sla_plan_updated_successfully'));
        } catch (Exception $e) {
            /* redirect to Index page with Fails Message */
            return redirect('slaplans')->with('fails', Lang::get('lang.sla_plan_can_not_update').'<li>'.$e->getMessage().'</li>');
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if ($default_sla->sla == $id) {
            ...
            $dept = DB::table('core__departments')->where('slaplan_id', '=', $id)->update(['slaplan_id' => $default_sla->sla]);
            if ($dept > 0) {
                if ($dept > 1) {
                    $text_dept = 'Emails';
                } else {
            ...
            try {
                $slas->delete();
                /* redirect to Index page with Success Message */
                return redirect('slaplans')->with('success', Lang::get('lang.sla_plan_deleted_successfully').$message);
            } catch (Exception $e) {
                /* redirect to Index page with Fails Message */
                return redirect('slaplans')->with('fails', Lang::get('lang.sla_plan_can_not_delete').'<li>'.$e->getMessage().'</li>');
            }
        }
    }
}
