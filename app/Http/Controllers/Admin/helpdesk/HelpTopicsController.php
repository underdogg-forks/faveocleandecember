...
class HelpTopicsController extends Controller
{
    ...
    public function index(Help_topic $topic)
    {
        try {
            $topics = $topic->get();
            return view('themes.default1.admin.helpdesk.manage.helptopics.index', compact('topics'));
        } catch (Exception $e) {
    ...
    public function create(Ticket_Priority $priority, Department $department, Help_topic $topic, Forms $form, User $agent, Sla_plan $sla)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.manage.helptopics.create', compact('priority', 'departments', 'topics', 'forms', 'agents', 'slas'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function store(Help_topic $topic, HelptopicRequest $request)
    {
        try {
            ...
            return redirect('helptopics')->with('success', Lang::get('lang.helptopic_created_successfully'));
        } catch (Exception $e) {
            /* redirect to Index page with Fails Message */
            return redirect('helptopics')->with('fails', Lang::get('lang.helptopic_can_not_create').'<li>'.$e->getMessage().'</li>');
        }
    }
    ...
    public function edit($id, Ticket_Priority $priority, Department $department, Help_topic $topic, Forms $form, Sla_plan $sla)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.manage.helptopics.edit', compact('priority', 'departments', 'topics', 'forms', 'agents', 'slas', 'sys_help_topic'));
        } catch (Exception $e) {
            return redirect('helptopics')->with('fails', '<li>'.$e->getMessage().'</li>');
        }
    }
    ...
    public function update($id, Help_topic $topic, HelptopicUpdate $request)
    {
        try {
            ...
            $topics->save();
            if ($request->input('sys_help_topic') == 'on') {
                \DB::table('tickets__settings')
                    ->where('id', '=', 1)
                    ->update(['tickets__helptopics' => $id]);
            }
            /* redirect to Index page with Success Message */
            return redirect('helptopics')->with('success', Lang::get('lang.helptopic_updated_successfully'));
        } catch (Exception $e) {
            /* redirect to Index page with Fails Message */
            return redirect('helptopics')->with('fails', Lang::get('lang.helptopic_can_not_update').'<li>'.$e->getMessage().'</li>');
        }
    }
    ...
    public function destroy($id, Help_topic $topic, Ticket $ticket_setting)
    {
        ...
        if ($ticket_settings->help_topic == $id) {
            ...
            try {
                $topics->delete();
                /* redirect to Index page with Success Message */
                return redirect('helptopics')->with('success', Lang::get('lang.helptopic_deleted_successfully').$message);
            } catch (Exception $e) {
                /* redirect to Index page with Fails Message */
                return redirect('helptopics')->with('fails', Lang::get('lang.helptopic_can_not_update').'<li>'.$e->getMessage().'</li>');
            }
