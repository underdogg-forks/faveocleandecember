...
class DepartmentController extends Controller
{
    ...
    {
        try {
            $department->fill($request->except('group_id', 'manager', 'slaplan_id'))->save();
            if ($request->slaplan_id) {
                $department->slaplan_id = $request->input('slaplan_id');
            } else {
                $department->slaplan_id = null;
            }
            ...
            if ($department->save() == true) {
                if ($request->input('sys_department') == 'on') {
                    DB::table('core__systemsettings')
                            ->where('id', 1)
                            ->update(['core__departments' => $department->id]);
                }
    ...
    {
        try {
            $sys_department = \DB::table('core__systemsettings')
                    ->select('department')
                    ->where('id', '=', 1)
                    ->first();
            $slas = $sla->where('status', '=', 1)
    ...
    public function update($id, Group_assign_department $group_assign_department, Department $department, DepartmentUpdate $request)
    {
        ...
        try {
            ...
            $departments->save();
            if ($request->slaplan_id) {
                $departments->slaplan_id = $request->input('slaplan_id');
                $departments->save();
            } else {
                $departments->slaplan_id = null;
                $departments->save();
            }
            if ($request->input('sys_department') == 'on') {
                DB::table('core__systemsettings')
                        ->where('id', 1)
                        ->update(['core__departments' => $id]);
            }
            if ($departments->fill($request->except('group_access', 'manager', 'slaplan_id'))->save()) {
    ...
    public function destroy($id, Department $department, Group_assign_department $group_assign_department, System $system, Tickets $tickets)
    {
        ...
        if ($system->department == $id) {
            ...
            $emails = DB::table('emails')->where('core__departments', '=', $id)->update(['core__departments' => $system->department]);
            if ($emails > 0) {
                if ($emails > 1) {
                    $text_emails = 'Emails';
                } else {
            ...
            $helptopic = DB::table('tickets__helptopics')->where('core__departments', '=', $id)->update(['core__departments' => null], ['status' => '1']);
            if ($helptopic > 0) {
